...
function tube.take(self, timeout, opts)
    ...
    while timeout > 0 do
        ...
        conds[fid] = {}
        local utube_name = opts ~= nil and opts.utube ~= nil and opts.utube or ""
        conds[fid][utube_name] = {}

        box.space._queue_consumers:insert{conn_id, fid, tid, time, started}
        
        conds[fid][utube_name][tid]= qc.waiter()
        conds[fid][utube_name][tid]:wait(tonumber(timeout) / 1000000)
        conds[fid][utube_name][tid]:free()
...
local function make_self(driver, space, tube_name, tube_type, tube_id, opts)
    ...
    local on_task_change = function(task, stats_data)
        ...
        if task[2] == state.READY then
            local utube_name = (task[9] ~= nil and task[8]) or (task[4] ~= nil and task[3]) or ""
            local tube_id = self.tube_id
            for s, consumer in queue_consumers.index.consumer:pairs{tube_id} do
                if conds[consumer[2]] ~= nil and conds[consumer[2]][utube_name] ~= nil then
                    local cond = conds[consumer[2]][utube_name][tube_id]
                    if cond ~= nil and consumer[3] == tube_id then
...
function method._on_consumer_disconnect()
    ...
    while true do
        ...
        for utube_name, ind in pairs(conds[waiter[2]]) do
            for tube_id, cond in pairs(conds[waiter[2]][utube_name]) do
                if cond then
